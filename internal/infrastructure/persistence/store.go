...
func NewStore(db *sql.DB, driver, schema, runtimeID, workerID string) (*Store, error) {
	if db == nil {
	...
	return &Store{db: db, r: r, runtimeID: runtimeID, workerID: workerID}, nil
}
...
func (s *Store) Reconcile(ctx context.Context, d schedulersdk.Definition, next time.Time) error {
	...
	changed, changedArgs, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_schedule_state").
	...
	stable, stableArgs, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_schedule_state").
	...
	insert, insertArgs, err := ormbuilder.NewInsertBuilder(s.r.Renderer, "scheduler_schedule_state").Columns("runtime_id", "definition_key", "revision", "enabled", "definition_json", "next_run_at", "snapshot_revision", "updated_at").Values(s.runtimeID, d.Key, d.Revision, true, string(raw), formatTime(next), int64(0), now).Build()
	if err != nil {
		return err
	}
...
func (s *Store) DisableMissing(ctx context.Context, active []string, revision int64) error {
	update, args, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_schedule_state").Set("enabled", false).Set("snapshot_revision", revision).Where(ormbuilder.Equal("runtime_id", s.runtimeID)).Build()
	if err != nil {
		return err
	}
	...
	update, args, err = ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_schedule_state").Set("enabled", true).Set("snapshot_revision", revision).Where(ormbuilder.And(ormbuilder.Equal("runtime_id", s.runtimeID), ormbuilder.In("definition_key", values...))).Build()
	if err != nil {
		return err
	}
...
func (s *Store) Due(ctx context.Context, now time.Time, limit int) ([]modulehost.DueTrigger, error) {
	...
	at := formatTime(now)
	query, args, err := ormbuilder.NewSelectBuilder(s.r.Renderer, "scheduler_schedule_state").Columns("definition_json", "next_run_at").Where(ormbuilder.And(ormbuilder.Equal("runtime_id", s.runtimeID), ormbuilder.Equal("enabled", true), ormbuilder.LessThanOrEqual("next_run_at", at))).OrderBy(ormbuilder.Ascending("next_run_at")).Limit(limit).Build()
	if err != nil {
		return nil, err
	}
	...
	retryQuery, retryArgs, err := ormbuilder.NewSelectBuilder(s.r.Renderer, "scheduler_runs").Columns("definition_key", "scheduled_for").Where(ormbuilder.And(ormbuilder.Equal("runtime_id", s.runtimeID), ormbuilder.Equal("status", "retrying"), ormbuilder.LessThanOrEqual("next_retry_at", at))).OrderBy(ormbuilder.Ascending("next_retry_at")).Limit(remaining).Build()
	if err != nil {
		return nil, err
	}
...
func (s *Store) takeExpired(ctx context.Context, id string, ttl time.Duration, now time.Time) (schedulersdk.Run, bool, error) {
	...
	update, args, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_runs").Set("status", "leased").Set("lease_owner", s.workerID).Set("lease_expires_at", formatTime(now.Add(ttl))).SetExpression("fencing_token", ormbuilder.Add(ormbuilder.Column("fencing_token"), ormbuilder.Value(1))).SetExpression("attempt", ormbuilder.Add(ormbuilder.Column("attempt"), ormbuilder.Value(1))).Set("updated_at", formatTime(now)).Where(predicate).Build()
	if err != nil {
		return schedulersdk.Run{}, false, err
	}
...
func (s *Store) Renew(ctx context.Context, run schedulersdk.Run, ttl time.Duration) (schedulersdk.Run, bool, error) {
	...
	update, args, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_runs").Set("lease_expires_at", formatTime(now.Add(ttl))).Set("updated_at", formatTime(now)).Where(predicate).Build()
	if err != nil {
...
func (s *Store) Fail(ctx context.Context, run schedulersdk.Run, failure error, retryAt time.Time) error {
	...
	builder := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_runs").Set("status", status).Set("last_error", failure.Error()).Set("lease_owner", nil).Set("lease_expires_at", nil).Set("updated_at", formatTime(time.Now())).Where(s.liveLease(run))
	if terminal {
	...
	if terminal {
		insert, insertArgs, buildErr := ormbuilder.NewInsertBuilder(s.r.Renderer, "scheduler_dead_letters").Columns("runtime_id", "run_id", "definition_key", "reason", "failed_at").Values(s.runtimeID, run.Trigger.RunID, run.Trigger.DefinitionKey, failure.Error(), formatTime(time.Now())).Build()
		if buildErr != nil {
			return buildErr
		}
...
func (s *Store) List(ctx context.Context, limit int) ([]schedulersdk.Run, error) {
	...
	query, args, err := ormbuilder.NewSelectBuilder(s.r.Renderer, "scheduler_runs").Columns("run_id").Where(ormbuilder.Equal("runtime_id", s.runtimeID)).OrderBy(ormbuilder.Descending("created_at")).Limit(limit).Build()
	if err != nil {
		return nil, err
	}
	rows, err := s.db.QueryContext(ctx, query, args...)
...
func (s *Store) finish(ctx context.Context, run schedulersdk.Run, status, receipt, lastError string) error {
	update, args, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_runs").Set("status", status).Set("receipt_json", nullable(receipt)).Set("last_error", nullable(lastError)).Set("lease_owner", nil).Set("lease_expires_at", nil).Set("updated_at", formatTime(time.Now())).Where(s.liveLease(run)).Build()
	if err != nil {
		return err
	}
	tx, err := s.db.BeginTx(ctx, nil)
...
func (s *Store) definition(ctx context.Context, key string) (schedulersdk.Definition, bool, error) {
	query, args, err := ormbuilder.NewSelectBuilder(s.r.Renderer, "scheduler_schedule_state").Columns("definition_json").Where(s.definitionIdentity(key)).Build()
	if err != nil {
...
func (s *Store) advanceCursorWith(ctx context.Context, executor sqlExecutor, d schedulersdk.Definition, scheduled, now time.Time, status string) error {
	...
	update, args, err := ormbuilder.NewUpdateBuilder(s.r.Renderer, "scheduler_schedule_state").Set("next_run_at", formatTime(next)).Set("last_run_at", formatTime(scheduled)).Set("last_run_status", status).Set("updated_at", formatTime(now)).Where(s.definitionIdentity(d.Key)).Build()
	if err != nil {
		return err
	}
...
func (s *Store) eventWith(ctx context.Context, executor sqlExecutor, runID, kind, message string) error {
	...
	insert, args, err := ormbuilder.NewInsertBuilder(s.r.Renderer, "scheduler_run_events").Columns("runtime_id", "event_id", "run_id", "event_type", "message", "created_at").Values(s.runtimeID, id, runID, kind, nullable(message), formatTime(time.Now())).Build()
	if err != nil {
		return err
	}
