...
func TestSchedulerDefinitionOwnershipMigrationAdoptsRuntimeCreatedTable(t *testing.T) {
	...
	if _, err := database.ExecContext(t.Context(), `CREATE TABLE scheduler_definitions (
	...
	if _, err := database.ExecContext(t.Context(), `INSERT INTO scheduler_definitions
	...
	var rows int
	if err := database.QueryRowContext(t.Context(), `SELECT COUNT(*) FROM scheduler_definitions WHERE resource_key = 'daily'`).Scan(&rows); err != nil {
		t.Fatal(err)
	}
	if rows != 1 {
